...
class Qwen3_5ForCausalLM(nn.Module):
    ...
    def forward(
        ...
        # DoLa: read candidate layers from env var (e.g. DOLA_LAYERS=3,7,11,15,19,23)
        import os
        _dola_layers = os.environ.get("DOLA_LAYERS", "")
        _dola_candidate_set = None
        ...
        if _dola_layers and forward_batch.forward_mode.is_decode_or_idle():
            ...
            # Clear previous DoLa data to prevent memory leak
            if hasattr(forward_batch, "dola_candidate_hidden") and forward_batch.dola_candidate_hidden is not None:
                forward_batch.dola_candidate_hidden.clear()
            else:
                forward_batch.dola_candidate_hidden = {}
            if hasattr(forward_batch, "dola_candidate_residual") and forward_batch.dola_candidate_residual is not None:
                forward_batch.dola_candidate_residual.clear()
            else:
            ...
            # Clear DoLa data for idle to avoid CUDA graph issues
            if hasattr(forward_batch, "dola_candidate_hidden"):
                forward_batch.dola_candidate_hidden = None
            if hasattr(forward_batch, "dola_candidate_residual"):
                forward_batch.dola_candidate_residual = None
        ...
        for layer_idx in range(len(self.layers)):
            ...
            # DoLa: capture candidate layer hidden states (after layer output, before norm)
            if _is_dola_active and layer_idx in _dola_candidate_set:
                forward_batch.dola_candidate_hidden[layer_idx] = hidden_states.clone()
                forward_batch.dola_candidate_residual[layer_idx] = residual.clone() if residual is not None else None
                forward_batch.dola_norm = self.norm  # pass norm reference for DoLa contrast
